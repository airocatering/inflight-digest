---
title: AirBaltic Slashes Fleet, Trading Growth for (Potential) Profitability
source: PaxEx.aero
link: https://paxex.aero/airbaltic-slashes-fleet-trading-growth-for-potential-profitability/
date: 2026-08-11
rubric: cabin-seating
image: https://paxex.aero/wp-content/uploads/2025/01/2025_01_29_airBaltic_Strategic-Partnership_1-scaled.jpg
slug: 2026-08-11-airbaltic-slashes-fleet-trading-growth-for-potential-profita
status: draft
---

AirBaltic, once hyper-focused on growth at all costs, now recognizes that it cannot hit its projected targets. The carrier will slash its A220 fleet from the current 54 to just 36 as part of a massive reorganization.

<!-- Ваш абзац. Что это значит для отрасли — два-три предложения.
     Без него материал выглядит как перепечатка. Строку с комментарием
     можно удалить. -->
